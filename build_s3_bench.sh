...
struct app_ctx {
    ...
    /* Endpoint override parsed from AWS_ENDPOINT_URL (if set) */
...
static void set_common_headers(struct aws_http_message *message, const char *host, const char *accept) {
    ...
    if (accept && accept[0]) {
        struct aws_http_header accept_header = {
            ...
            .value = aws_byte_cursor_from_c_str(accept),
        };
    ...
    struct aws_http_header ua_header = {
        ...
        .value = aws_byte_cursor_from_c_str("aws-c-s3-benchmark/1.0"),
    };
    aws_http_message_add_header(message, ua_header);
}

static void build_aws_host(char *out, size_t out_sz, const char *bucket, const char *region) {
    /* Virtual-hosted style for AWS S3 */
    snprintf(out, out_sz, "%s.s3.%s.amazonaws.com", bucket, region);
}

static void build_list_path(char *out, size_t out_sz, int use_virtual_host, const char *bucket, const char *prefix, const char *continuation_token) {
    if (use_virtual_host) {
        if (continuation_token && continuation_token[0])
            snprintf(out, out_sz, "/?list-type=2&prefix=%s&continuation-token=%s", prefix, continuation_token);
        else
            snprintf(out, out_sz, "/?list-type=2&prefix=%s", prefix);
    } else {
        if (continuation_token && continuation_token[0])
            snprintf(out, out_sz, "/%s/?list-type=2&prefix=%s&continuation-token=%s", bucket, prefix, continuation_token);
        else
            snprintf(out, out_sz, "/%s/?list-type=2&prefix=%s", bucket, prefix);
    }
}

static void build_get_path(char *out, size_t out_sz, int use_virtual_host, const char *bucket, const char *key) {
    if (use_virtual_host) {
        snprintf(out, out_sz, "/%s", key);
    } else {
        snprintf(out, out_sz, "/%s/%s", bucket, key);
    }
}
...
static int parse_endpoint_override(struct app_ctx *app) {
    ...
    if (!endpoint_env || !endpoint_env[0]) {
        ...
        app->endpoint_authority[0] = '\0';
        return 0;
    }

    struct aws_byte_cursor endpoint_cur = aws_byte_cursor_from_c_str(endpoint_env);
    if (aws_uri_init_parse(&app->endpoint_uri, app->allocator, &endpoint_cur)) {
        fprintf(stderr, "Failed to parse AWS_ENDPOINT_URL: %s\n", endpoint_env);
        app->has_endpoint_override = 0;
        app->endpoint_authority[0] = '\0';
        return -1;
    }

    const struct aws_byte_cursor *authority = aws_uri_authority(&app->endpoint_uri);
    if (!authority || authority->len == 0) {
        fprintf(stderr, "Invalid AWS_ENDPOINT_URL (no authority): %s\n", endpoint_env);
        aws_uri_clean_up(&app->endpoint_uri);
        app->has_endpoint_override = 0;
        app->endpoint_authority[0] = '\0';
        return -1;
    }

    size_t copy_len = authority->len < sizeof(app->endpoint_authority) - 1 ? authority->len : sizeof(app->endpoint_authority) - 1;
    memcpy(app->endpoint_authority, authority->ptr, copy_len);
    app->endpoint_authority[copy_len] = '\0';
...
static int list_objects(struct aws_s3_client *client, struct aws_allocator *allocator,
    const char *bucket_name, const char *region, const char *prefix, struct object_list *obj_list,
    const struct aws_signing_config_aws *signing_config, struct app_ctx *app) {
    ...
    while (has_more) {
        ...
        /* Build host and path */
        char host[512];
        int use_virtual_host = app->has_endpoint_override ? 0 : 1;
        if (app->has_endpoint_override) {
            snprintf(host, sizeof(host), "%s", app->endpoint_authority);
        } else {
            build_aws_host(host, sizeof(host), bucket_name, region);
        }

        char path[2048];
        build_list_path(path, sizeof(path), use_virtual_host, bucket_name, prefix, continuation_token);
        ...
        set_common_headers(message, host, "application/xml");

        struct aws_s3_meta_request_options options;
        AWS_ZERO_STRUCT(options);
        options.type = AWS_S3_META_REQUEST_TYPE_DEFAULT;
        options.message = message;
        options.body_callback = on_list_body;
        options.finish_callback = on_list_finished;
        options.user_data = &list_ctx;
        options.signing_config = signing_config;
        if (app->has_endpoint_override) {
            options.endpoint = &app->endpoint_uri;
        }
...
int main(int argc, char *argv[]) {
    ...
    /* Parse endpoint override if provided (needed for http endpoints like MinIO) */
    ...
    if (list_objects(client, allocator, bucket_name, region, prefix, &obj_list, &signing_config, &app) != 0) {
    ...
    for (int i = 0; i < files_to_download; i++) {
        ...
        /* Build host and path (virtual host for AWS, path-style when endpoint override is used) */
        char host[512];
        int use_virtual_host = app.has_endpoint_override ? 0 : 1;
        if (app.has_endpoint_override) {
            snprintf(host, sizeof(host), "%s", app.endpoint_authority);
        ...
        char uri[2048];
        build_get_path(uri, sizeof(uri), use_virtual_host, bucket_name, ctx->object_key);

        struct aws_http_message *message = aws_http_message_new_request(allocator);
        aws_http_message_set_request_method(message, aws_http_method_get);
        aws_http_message_set_request_path(message, aws_byte_cursor_from_c_str(uri));
        set_common_headers(message, host, "*/*");

        struct aws_s3_meta_request_options options;
        AWS_ZERO_STRUCT(options);
        options.type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT;
        options.message = message;
        options.body_callback = on_body_received;
        options.finish_callback = on_request_finished;
